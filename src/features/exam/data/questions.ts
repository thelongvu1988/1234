...
export const allQuestions = questionsJson as Question[]

export const A_A1_QUESTION_IDS = [
...
const getQuestionsByIds = (ids: number[]): Question[] => {
  const idSet = new Set(ids)
  return allQuestions.filter((question) => idSet.has(Number(question.id)))
}

export const getQuestionsByLicense = (licenseType: LicenseType): Question[] => {
  if (licenseType === 'A1' || licenseType === 'A') {
    return getQuestionsByIds(A_A1_QUESTION_IDS)
  }

  if (licenseType === 'B1') {
    return getQuestionsByIds(B1_QUESTION_IDS)
  }
